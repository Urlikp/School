...
// ======== BEGIN OF SOLUTION - TASK 2_2-1 ======== //
  ...
  // speed should not exceed the SPACESHIP_SPEED_MAX
    ...
    gameObjects.spaceShip->speed = std::min(gameObjects.spaceShip->speed + deltaSpeed, SPACESHIP_SPEED_MAX);
...
// ======== BEGIN OF SOLUTION - TASK 2_2-2 ======== //
  ...
  // speed have to be always greater or equal to zero
    ...
    gameObjects.spaceShip->speed = std::max(gameObjects.spaceShip->speed - deltaSpeed, 0.0f);
...
// ======== BEGIN OF SOLUTION - TASK 2_2-3 ======== //
  ...
  // update space ship direction according to the view angle
    ...
    gameObjects.spaceShip->direction = glm::vec3(
        cos(glm::radians(gameObjects.spaceShip->viewAngle)), 
        sin(glm::radians(gameObjects.spaceShip->viewAngle)), 
        0.0f
    );
...
// ======== BEGIN OF SOLUTION - TASK 2_2-5 ======== //
    ...
    // define a variable with camera view direction 
    //   it will be used for computation of cameraCenter in task 2-6 and elevated in a 3D view )
...
// ======== BEGIN OF SOLUTION - TASK 2_3-1 ======== //
    ...
    //  - prepare rotation axis as normal of plane defined by camera view and up vectors
    //  - prepare matrix for rotation around this axis by camera elevation angle
    //  - rotate both vectors around rotation axis using this matrix  

    glm::vec3 rotAxis = glm::cross(gameObjects.spaceShip->direction, cameraUpVector);

    glm::mat4 rotMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(gameState.cameraElevationAngle), rotAxis);

    glm::vec3(rotMatrix * glm::vec4(gameObjects.spaceShip->direction, 0.0f));
...
// ======== BEGIN OF SOLUTION - TASK 2_2-6 ======== //
    // compute camera center from camera position and camera view direction
    glm::vec3 cameraCenter = gameObjects.spaceShip->position + glm::vec3(rotMatrix * glm::vec4(gameObjects.spaceShip->direction, 0.0f));
...
// ======== BEGIN OF SOLUTION - TASK 2_2-7 ======== //
  // update space ship position taking into account timeDelta, space ship speed, and space ship direction
    ...
    gameObjects.spaceShip->position += gameObjects.spaceShip->direction * timeDelta * gameObjects.spaceShip->speed;
