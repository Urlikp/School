...
// ======== BEGIN OF SOLUTION - TASK 3_3-2 ======== //
  ...
  if (dotAngle > light.spotCosCutOff) {
    ret += material.diffuse * light.diffuse * max(dot(L, vertexNormal), 0.0);
    ret += material.specular * light.specular * pow(max(dot(R, V), 0.0), material.shininess);
    ret += material.ambient * light.ambient;
    ret *= pow(max(dotAngle, 0.0), light.spotExponent);
  }
...
// ======== BEGIN OF SOLUTION - TASK 3_3-3 ======== //
  ...
  spaceShipReflector.spotDirection = (Vmatrix * vec4(reflectorDirection, 0.0)).xyz;
