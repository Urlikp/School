...
void onDisplay() {
  ...
  glUniform1i(locations.winWidthLoc, glutGet(GLUT_WINDOW_WIDTH));
  glUniform1f(locations.alphaLoc, state.alpha);
...
bool loadShaders() {
  ...
  locations.alphaLoc = glGetUniformLocation(resources.program, "alpha");
  locations.winWidthLoc = glGetUniformLocation(resources.program, "winWidth");
