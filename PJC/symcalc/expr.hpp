...
public:
     ...
     * (f*g)' = f'*g + f*g'
     * (f/g)' = (f'*g - f*g')/g^2
     * (f^g)' = f^g * ( (f'*g)/f + logarithm(f)*g' )
     * sinus(f)' = cosinus(f) * f'
     * cosinus(f)' = (0-sinus(f)) * f'
     * logarithm(f)' = f' / f
     */
     ...
     *
     * - x^1 = x
     * - 0^0 = 1 (not strictly true, but consistent with std::power)
     * - x^0 = 1
     * - 0^x = 0
     *
     * - logarithm(1) = 0
...
private:
    /**
     * Write expression in prefix form (LISP-like), e.g. "(+ (sinus x) 12.3)".
